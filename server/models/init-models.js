...
function initModels(sequelize) {
	...
	order.belongsToMany(image, {
		as: 'image_id_image_order_images',
		through: order_image,
		foreignKey: 'order_id',
		otherKey: 'image_id',
	});
	tag.belongsToMany(image, {
		as: 'image_id_image_image_tags',
		through: image_tag,
		foreignKey: 'tag_id',
		otherKey: 'image_id',
	});
	user.belongsToMany(offer, {
		as: 'offer_id_offer_user_offers',
		through: user_offer,
		foreignKey: 'user_id',
		otherKey: 'offer_id',
	});
	image_offer.belongsTo(image, {
		as: 'image',
		foreignKey: 'image_id',
		onDelete: 'CASCADE',
	});
	image.hasMany(image_offer, { as: 'image_offers', foreignKey: 'image_id' });
	image_tag.belongsTo(image, {
		as: 'image',
		foreignKey: 'image_id',
		onDelete: 'CASCADE',
	});
	image.hasMany(image_tag, { as: 'image_tags', foreignKey: 'image_id' });
	order_image.belongsTo(image, {
		as: 'image',
		foreignKey: 'image_id',
		onDelete: 'CASCADE',
	});
	image.hasMany(order_image, { as: 'order_images', foreignKey: 'image_id' });
	shopping_cart_image.belongsTo(image, {
		as: 'image',
		foreignKey: 'image_id',
		onDelete: 'CASCADE',
	});
	image.hasMany(shopping_cart_image, {
		as: 'shopping_cart_images',
		foreignKey: 'image_id',
	});
	version.belongsTo(image, {
		as: 'image',
		foreignKey: 'image_id',
		onDelete: 'CASCADE',
	});
	image.hasMany(version, { as: 'versions', foreignKey: 'image_id' });
	version.belongsTo(image, {
		as: 'original',
		foreignKey: 'original_id',
		onDelete: 'CASCADE',
	});
	image.hasMany(version, {
		as: 'original_versions',
		foreignKey: 'original_id',
	});
	...
	order_image.belongsTo(order, {
		as: 'order',
		foreignKey: 'order_id',
		onDelete: 'CASCADE',
	});
	order.hasMany(order_image, { as: 'order_images', foreignKey: 'order_id' });
	shopping_cart_image.belongsTo(shopping_cart, {
		as: 'shopping_cart',
		foreignKey: 'shopping_cart_id',
		onDelete: 'CASCADE',
	});
	shopping_cart.hasMany(shopping_cart_image, {
		as: 'shopping_cart_images',
		foreignKey: 'shopping_cart_id',
	});
	image_tag.belongsTo(tag, {
		as: 'tag',
		foreignKey: 'tag_id',
		onDelete: 'CASCADE',
	});
	tag.hasMany(image_tag, { as: 'image_tags', foreignKey: 'tag_id' });
	image.belongsTo(technical_metadata, {
		as: 'technical_metadatum',
		foreignKey: 'technical_metadata_id',
		onDelete: 'SET NULL',
	});
	technical_metadata.hasMany(image, {
		as: 'images',
		foreignKey: 'technical_metadata_id',
		onDelete: 'SET NULL',
	});
